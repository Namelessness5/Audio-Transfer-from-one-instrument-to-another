...
class Generator(nn.Module):
    ...
    def __init__(self, conv_dim=64, num_speakers=10, repeat_num=6):
        ...
        c_dim = num_speakers
        layers = []
        layers.append(nn.Conv2d(1+c_dim, conv_dim, kernel_size=3, padding=1, bias=False))
        ...
        curr_dim = conv_dim
        for i in range(2):
            layers.append(nn.Conv2d(curr_dim, curr_dim*2, kernel_size=(8, 4), stride=(2, 2), padding=(3, 1), bias=False))
...
class Discriminator(nn.Module):
    ...
    def __init__(self, input_size=(84, 188), conv_dim=32, repeat_num=5, num_speakers=10, f_layer=2):
        super(Discriminator, self).__init__()
        # discriminator
        layers = []
        layers.append(nn.Conv2d(1+num_speakers, conv_dim, kernel_size=3, stride=1, padding=1))
        ...
        for i in range(f_layer, repeat_num):
            layers.append(ResidualBlock(dim_in=curr_dim, dim_out=curr_dim))
        ...
        kernel_size_0 = int(input_size[0] / np.power(2, f_layer)) # 1
        kernel_size_1 = int(input_size[1] / np.power(2, f_layer)) # 8
        self.mainD = nn.Sequential(*layers)
    ...
    def forward(self, x, c):
        c = c.view(c.size(0), c.size(1), 1, 1)
        c = c.repeat(1, 1, x.size(2), x.size(3))
        x1 = torch.cat([x, c], dim=1)
        h = self.mainD(x1)
        out_src = self.conv_dis(h)

        h2 = self.mainC(x)
        out_cls_spks = self.conv_clf_spks(h2)
        return out_src.view(out_src.size(0), out_src.size(1)), out_cls_spks.view(out_cls_spks.size(0), out_cls_spks.size(1))
